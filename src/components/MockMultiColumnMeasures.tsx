...
export default function MockMultiColumnMeasures({ onToggleMock }: { onToggleMock?: () => void }) {
  ...
  >(
    ALL_TERMS.slice(0, 20).map((term, index) => ({
  ...
  const handleLibrarySelect = (term: { id: string; label: string }) => {
    if (activeCardIdForSelection) {
      setCards(
        cards.map((c) =>
          c.id === activeCardIdForSelection ? { ...c, termId: term.id, termLabel: term.label } : c
        )
      );
    }
  ...
  const toggleCardViewMode = (cardId: string) => {
    setCards(
      cards.map((c) =>
        c.id === cardId ? { ...c, viewMode: c.viewMode === 'list' ? 'table' : 'list' } : c
      )
    );
  };
                  ...
                  >
                      ...
                      title={
                        isConfigured ? (
                          <div className="flex items-center gap-2">
                            ...
                            {abbr && (
                              <Chip label={abbr} size="small" color="primary" variant="outlined" />
                            )}
                            <Button
                              size="small"
                              ...
                              onChange={(_, newValue) => {
                                if (newValue) {
                                  setCards(
                                    cards.map((c) =>
                                      c.id === card.id
                                        ? { ...c, termId: newValue.id, termLabel: newValue.label }
                                        : c
                                    )
                                  );
                                }
                      ...
                      action={
                        <div className="flex gap-1">
                          <Tooltip
                            title={
                              card.viewMode === 'list'
                                ? 'Switch to Table View (Edit Details)'
                                : 'Switch to Chip View'
                            }
                          >
                            <IconButton
                              onClick={() => toggleCardViewMode(card.id)}
                              color={card.viewMode === 'table' ? 'primary' : 'default'}
                            >
                    ...
                    <CardContent>
                      <div className="flex justify-between items-center mb-2">
                        <Typography
                          variant="subtitle2"
                          className="text-gray-600 font-bold uppercase text-xs tracking-wider"
                        >
                                ...
                                return (
                                  <TableRow key={colId}>
                                    ...
                                    <TableCell className="align-top">
                                      <div className="w-full mt-1">
                                        <DescriptionEditor
                                          ...
                                          onDescriptionChange={(_, val) =>
                                            updateColumnMetadata(colId, 'description', val)
                                          }
                                          columnID={colId}
                                        />
                                      </div>
                                    ...
                                    <TableCell className="align-top pt-3">
                                      ...
                                      >
                                        <Tooltip title="Categorical" arrow>
                                          <ToggleButton
                                            value="Categorical"
                                            className="flex-1 w-1/2 px-2 py-1"
                                          >
                                    ...
                                    <TableCell align="right" className="align-top pt-3">
                                      <IconButton
                                        size="small"
                                        onClick={() => handleUnassignColumn(card.id, colId)}
                                      >
