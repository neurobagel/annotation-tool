...
describe('Main user flow', () => {
  ...
  it.only('steps through the different app workflows with a partially annotated data dictionary', () => {
    ...
    cy.get('[data-cy="2-M-term-dropdown"]').type('Male{downArrow}{enter}');
    cy.get('[data-cy="2-F-description"]').should('be.visible');
    ...
    cy.get('[data-cy="3-ADHD-description"]').type('Attention deficit hyperactivity disorder');
    cy.get('[data-cy="3-ADHD-description"]').should(
      'contain',
      'Attention deficit hyperactivity disorder'
    );
    cy.get('[data-cy="3-ADHD-term-dropdown"]').type(
      'Attention deficit hyperactivity disorder{downArrow}{enter}'
    );
    cy.get('[data-cy="3-PD-description"]').type('Parkinsons');
    cy.get('[data-cy="3-PD-description"]').should('contain', 'Parkinsons');
    cy.get('[data-cy="3-PD-term-dropdown"]').type(
    ...
    cy.get('[data-cy="4-HC-term-dropdown"]').type('Healthy{downArrow}{enter}');
    cy.get('[data-cy="4-N/A-missing-value-yes"]').click();
    cy.get('[data-cy="4-Patient-description"]').type('Patient');
    cy.get('[data-cy="4-Patient-description"]').should('contain', 'Patient');
    ...
    const outputFileName = `${mockDataDictionaryFileName.split('.')[0]}_annotated.json`;
    const outputPath = `cypress/downloads/${outputFileName}`;
    ...
    // Upload view - Use the freshly downloaded file for the second upload
    cy.get('[data-cy="datatable-upload-input"]').selectFile(mockDataTableFilePath, {
      force: true,
    });
    cy.get('[data-cy="datadictionary-upload-input"]').selectFile(outputPath, {
      force: true,
    });
    cy.get('[data-cy="next-button"]').click();
    ...
    cy.get('[data-cy="2-M-term-dropdown"] input').should('have.value', 'Male');
    cy.get('[data-cy="2-F-description"]').should('contain', 'Female');
    cy.get('[data-cy="2-F-term-dropdown"] input').should('have.value', 'Female');
    cy.get('[data-cy="side-column-nav-bar-diagnosis-group_dx"]').should('be.visible');
    cy.get('[data-cy="side-column-nav-bar-diagnosis-select-button"]').click();
    cy.get('[data-cy="3-ADHD-description"]').should(
      'contain',
    ...
    cy.get('[data-cy="3-ADHD-term-dropdown"] input').should(
      'have.value',
      'Attention deficit hyperactivity disorder'
    );
    cy.get('[data-cy="3-PD-description"]').should('contain', 'Parkinsons');
    cy.get('[data-cy="3-PD-term-dropdown"] input').should(
      'have.value',
    ...
    cy.get('[data-cy="dataset-references-input-0"] input').type('https://domain.com/paper');
